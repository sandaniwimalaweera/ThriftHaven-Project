...
// Admin contact info (only shown on collected donations)
...
const DonationCard = ({ 
  ...
  // Inject admin info if collected and not already present
  if (isCollected && !donation.adminInfo) {
    donation.adminInfo = ADMIN_CONTACT_INFO;
  }
...
{/*Admin Info Section */}
{donation.collection_status === "collected" && donation.adminInfo && (
  <Box sx={{ mt: 3, p: 2, bgcolor: "#f3e5f5", borderRadius: 2 }}>
    ...
    <Typography variant="body2"><strong>Address:</strong> {donation.adminInfo.address}</Typography>
    <Typography variant="body2"><strong>Contact:</strong> {donation.adminInfo.contact}</Typography>
  </Box>
)}
