...
const SellerSidebar = ({ userName, isSidebarOpen = true }) => {
  ...
  useEffect(() => {
    ...
    if (path === "/") {
        setActiveItem("home");
      } else if (path === "/auth/seller-dashboard") {
        if (hash === "donations") {
      ...
      } else if (path === "/auth/seller-order-details") {
  ...
  const navigateToSection = (section, itemName) => {
    ...
    router.push(`/auth/seller-dashboard#${section}`);
  };
  ...
  return (
    <Box sx={{ 
      ...
      >
        ...
        <Box mt={3} sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
          <List sx={{ width: "100%" }}>
            <ListItemButton 
            ...
            <ListItemButton 
              onClick={() => navigateToSection("products", "products")}
          ...
          >
            ...
            <ListItemButton 
             onClick={() => navigateTo("/auth/seller-order-details", "refundDetails")}
